...
type HandlerReturn<E> = Box<dyn Future<Output = Result<Request<hyper::body::Incoming>, E>> + Send + 'static>;
...
impl<E: Into<Box<dyn std::error::Error + Send + Sync>> + 'static> PreMiddleware<E> {
    ...
    where
        ...
        R: Future<Output = Result<Request<hyper::body::Incoming>, E>> + Send + 'static,
    {
    ...
    ) -> crate::Result<Request<hyper::body::Incoming>> {
        let handler = self
            .handler
            .as_ref()
